...
### String autocomplete
The mains string field uses the format **Manufacturer — Name — Gauge** (e.g. `Luxilon — ALU Power — 1.25`). As you type, previously used strings appear as suggestions. Consistent naming ensures accurate chart grouping.

### Hybrid setups and different mains/crosses tensions
For hybrid setups or jobs where mains and crosses are strung at different tensions, fill in the optional **Crosses string** and **Crosses tension** fields. The racket card and job card display both strings and tensions. The chart uses the average of mains and crosses tension for plotting hybrid jobs. The **Strings used** summary counts mains and crosses strings separately.
